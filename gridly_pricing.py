...
def calculate_gridly_pricing(plan, modules, total_platform_seats, total_translation_seats, billing_cycle):
    ...
    included_platform_seats = sum(
        module_pricing[module]["included_seats"].get(plan, 0)
        for module in modules if module in ["CMS", "TMS"]
    )
    included_translation_seats = sum(
        module_pricing[module]["included_seats"].get(plan, 0)
        for module in modules if module == "CAT"
    )
    ...
    for module in modules:
        if module in module_pricing:
            base_fee = module_pricing[module]["fee"].get(plan, 0)
